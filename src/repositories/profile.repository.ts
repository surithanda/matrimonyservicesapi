...
export class ProfileRepository {
  ...
  async getPersonalProfile(profileData: IProfilePersonal): Promise<any> {
    try {
      ...
      const params = [profileData.profile_id, null, profileData.created_user];
  ...
  async createPersonalProfile(profileData: IProfilePersonal): Promise<any> {
    try {
      ...
      const params = [
        ...
        profileData.phone_emergency === undefined
          ? null
        ...
        profileData.height_inches === undefined
          ? null
        ...
        profileData.weight_units === undefined
          ? null
        ...
        profileData.whatsapp_number === undefined
          ? null
  ...
  formatResponse(result: any, arrayElement?: string): any {
    ...
    if (!result || !Array.isArray(result) || result.length === 0) {
      ...
      returnObj = {
        ...
        ...(arrayElement
          ? { [arrayElement.toString()]: extractedResponse }
          : { ...extractedResponse }),
      };
    } else {
      returnObj = {
        status: "success",
        ...(arrayElement
          ? { [arrayElement.toString()]: (result as any[])[0] }
          : {}),
      };
    }
    return returnObj;
  ...
  async getProfilePhotos(profileId: number): Promise<any[]> {
    try {
      const [result] = await pool.execute("CALL eb_profile_photo_get(?)", [
        profileId,
      ]);
  ...
  async getProfileHobbies(profileData: IProfileHobbyInterest): Promise<any> {
    try {
      ...
      const [result] = await pool.execute(
        "CALL eb_profile_hobby_interest_get(?,?,?,?)",
        params
      );
  ...
  async addProfileHobby(hobbyData: IProfileHobbyInterest): Promise<any> {
    try {
      const params = [
        hobbyData.profile_id,
        hobbyData.hobby,
        hobbyData.created_user,
      ];
      const [result] = await pool.execute(
  ...
  async removeProfileHobby(hobbyData: IProfileHobbyInterest): Promise<any> {
    try {
      ...
      const [result] = await pool.execute(
        "CALL eb_profile_hobby_interest_remove(?, ?, ?)",
        params
      );
      return (result as any[])[0][0];
    } catch (error) {
  ...
  async deletePersonalProfile(deleteData: any): Promise<any> {
    try {
      const params = [deleteData.profile_id, deleteData.created_user];
  ...
  async getCompleteProfile(profileData: any): Promise<any> {
    try {
      const params = [profileData.profile_id, profileData.created_user];
  ...
  async getAllProfiles(profileData: any): Promise<any> {
    try {
      const params = [profileData.profile_id];
  ...
  async getProfileCompletionCount(profileData: any): Promise<any> {
    try {
      const params = [profileData.profile_id, profileData.user];
      const [result] = await pool.execute(
        "CALL eb_profile_related_data_count(?,?)",
        params
      );
